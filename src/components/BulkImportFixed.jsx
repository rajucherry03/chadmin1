...
const BulkImportFixed = ({ onClose, onSuccess }) => {
  ...
  const availableFields = [
    ...
    { name: "aadhaar", label: "Aadhaar", type: "text", required: false },
    { name: "studentMobile", label: "Student Mobile", type: "tel", required: false },
    { name: "fatherMobile", label: "Father Mobile", type: "tel", required: false },
    { name: "fatherName", label: "Father Name", type: "text", required: false },
    { name: "motherName", label: "Mother Name", type: "text", required: false },
    { name: "address", label: "Permanent Address", type: "textarea", required: false }
  ];
  ...
  const validateData = () => {
    ...
    if (!mapping.rollNo || !mapping.name) {
        newErrors.push({
          type: 'mapping',
        message: 'Roll No and Student Name are required fields'
        });
      }
    ...
    data.forEach((row, rowIndex) => {
      ...
      // Check required fields
      if (mapping.rollNo !== undefined && (!row[mapping.rollNo] || row[mapping.rollNo].toString().trim() === '')) {
            newErrors.push({
              type: 'validation',
              row: rowNumber,
          field: 'Roll No',
      ...
      if (mapping.name !== undefined && (!row[mapping.name] || row[mapping.name].toString().trim() === '')) {
              newErrors.push({
                type: 'validation',
                row: rowNumber,
          field: 'Student Name',
      ...
      if (mapping.studentMobile !== undefined && row[mapping.studentMobile]) {
        const phone = row[mapping.studentMobile].toString().replace(/\D/g, '');
      ...
      if (mapping.fatherMobile !== undefined && row[mapping.fatherMobile]) {
        const phone = row[mapping.fatherMobile].toString().replace(/\D/g, '');
        if (phone.length < 10 || phone.length > 11) {
              newErrors.push({
                type: 'validation',
                row: rowNumber,
      ...
      // Validate Aadhaar
      if (mapping.aadhaar !== undefined && row[mapping.aadhaar]) {
        const aadhaar = row[mapping.aadhaar].toString().replace(/\D/g, '');
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          studentData.year = 'A'; // Default year
          studentData.section = 'IV'; // Default section
          ...
          const studentId = `${deptShort}_${studentData.year}_${studentData.section}_${studentData.rollNo}`;
          ...
          try {
            ...
            if (!methods || methods.length === 0) {
              ...
              // Add auth info to student data
              finalStudentData.authUid = authUid;
              finalStudentData.authEmail = email;
              finalStudentData.authCreatedAt = serverTimestamp();
            } else {
              console.warn(`Auth account already exists for ${email}`);
            }
          } catch (authError) {
            console.warn(`Auth creation failed for ${email}:`, authError);
            // Continue without auth account
          }

          // Store in Firebase with correct path structure: /students/department/year/section/{UID}
          const sanitizedDept = getDepartmentShortName(studentData.department);
          const sanitizedYear = studentData.year;
          const sanitizedSection = studentData.section;
          
          // Use Firebase Auth UID as document ID if available, otherwise use roll number
          const documentId = authUid || studentData.rollNo;
          const studentDoc = doc(db, `students/${sanitizedDept}/${sanitizedYear}/${sanitizedSection}`, documentId);
          
          currentBatch.set(studentDoc, finalStudentData);

          // Also create a reference in studentsByUid if auth was created
          if (authUid) {
            ...
            currentBatch.set(byUidRef, {
              ...
              primaryDocPath: `students/${sanitizedDept}/${sanitizedYear}/${sanitizedSection}/${documentId}`,
              updatedAt: serverTimestamp()
            }, { merge: true });
          }

...
export const DjangoAuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (result.success) {
        // Use user data from login result if available, otherwise try to fetch from profile
        let userData = result.user;
        
        if (!userData || !userData.email) {
          console.log('No user data in login result, attempting to fetch from profile...');
          try {
            userData = await getDjangoCurrentUser();
          } catch (error) {
            console.warn('Could not fetch user profile, using fallback:', error);
        ...
        // If still no user data, create a basic one
        if (!userData) {
          userData = { 
            email: email,
            username: email.split('@')[0],
  ...
  const logout = async () => {
    try {
      ...
      console.log('Context: Logout result:', logoutResult);
  ...
  const refreshToken = async () => {
    try {
      console.log('Context: Starting token refresh...');
      ...
      if (refreshed) {
        ...
        console.warn('Context: Token refresh failed');
  ...
  const ensureValidToken = async () => {
    try {
      ...
      if (!isValid) {
        console.warn('Context: Token validation failed, clearing auth state');
        setUser(null);
        setIsAuthenticated(false);
      }
      return isValid;

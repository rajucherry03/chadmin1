...
const ReportsAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeFacultyDirectory = onSnapshot(
    ...
    const unsubscribeTeachingReports = onSnapshot(
      collection(db, "teachingReports"),
      (snapshot) => {
        const reports = [];
        snapshot.forEach((doc) => {
          reports.push({ id: doc.id, ...doc.data() });
        });
        setTeachingReports(reports);
      }
    );

    // Fetch performance reports
    const unsubscribePerformanceReports = onSnapshot(
    ...
    const unsubscribeComplianceReports = onSnapshot(
    ...
    return () => {
      ...
      unsubscribeTeachingReports();
      unsubscribePerformanceReports();
      unsubscribeComplianceReports();
    };
  }, []);

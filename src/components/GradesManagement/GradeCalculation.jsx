...
const GradeCalculation = ({ 
  ...
  const calculateAllGrades = async () => {
    ...
    try {
      ...
      toast.success(`Grades calculated for ${filteredStudents.length} students!`);
    } catch (error) {
  ...
  const exportGrades = () => {
    ...
    if (gradesToExport.length === 0) {
      toast.warning('No grades to export');
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-64">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading grade calculation data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <button
                ...
                disabled={saving}
                className="flex-1 bg-gradient-to-r from-purple-500 to-purple-600 text-white px-4 py-3 rounded-xl hover:from-purple-600 hover:to-purple-700 transition-all duration-200 font-medium disabled:opacity-50"
              >
                {saving ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">SGPA</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">CGPA</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">Credits</th>
                  ...
                  <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        sgpa.sgpa >= 8.0 ? 'bg-green-100 text-green-800' :
                        sgpa.sgpa >= 6.0 ? 'bg-blue-100 text-blue-800' :
                        sgpa.sgpa >= 4.0 ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        cgpa.cgpa >= 8.0 ? 'bg-green-100 text-green-800' :
                        cgpa.cgpa >= 6.0 ? 'bg-blue-100 text-blue-800' :
                        cgpa.cgpa >= 4.0 ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {cgpa.cgpa}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-900">{sgpa.totalCredits}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        calculatedGrade ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Average SGPA</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Calculated</p>
              <p className="text-3xl font-bold text-gray-900">{Object.keys(calculatedGrades).length}</p>
            </div>

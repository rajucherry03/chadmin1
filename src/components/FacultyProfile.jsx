...
const FacultyProfile = () => {
  ...
  const loadFacultyProfile = async () => {
    try {
      ...
      const result = await getFacultyProfile(currentUser.uid);
      
      if (result.success) {
        setFacultyData(result.data);
      } else {
        setError(result.error || 'Failed to load faculty profile');
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (result.success) {
        ...
        setEditing(false);
        await loadFacultyProfile(); // Reload to get updated data
      } else {
  ...
  const roleInfo = FACULTY_ROLES[facultyData.role] || FACULTY_ROLES["Lecturer"];
